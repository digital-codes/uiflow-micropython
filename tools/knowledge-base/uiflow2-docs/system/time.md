...
<!-- .. function:: gmtime([secs]) -->
    ...
    and `localtime()` returns a date-time tuple in local time.
...
<!-- .. function:: sleep_ms(ms) -->
    ...
    but may take longer than that if other processing must take place,
...
<!-- .. function:: ticks_ms() -->
    ...
    The wrap-around value is not explicitly exposed, but we will refer to
    it as *TICKS_MAX* to simplify discussion. Period of the values is
    *TICKS_PERIOD = TICKS_MAX + 1*. *TICKS_PERIOD* is guaranteed to be a power
    of two, but otherwise may differ from port to port. The same period value is
    used for all of `ticks_ms()`, `ticks_us()`, `ticks_cpu()` functions
...
<!-- .. function:: ticks_add(ticks, delta) -->
    ...
    definition of tick values (see `ticks_ms()` above). *ticks* parameter must
    be a direct result of call to `ticks_ms()`, `ticks_us()`, or `ticks_cpu()`
...
<!-- .. function:: ticks_diff(ticks1, ticks2) -->
    ...
    (or more specifically, ring) arithmetics to produce correct result even for
